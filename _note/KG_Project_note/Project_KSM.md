...
| LS_LB_SUM     | 대출현황_대출책수_계                 | x      | 0.577166    |
...
- **지금은 머신러닝 기반 모델 튜닝에 집중**
    
    - 유저 피처 변환 강화
        
    - 후처리 보정 로직 도입
        
    - SHAP/LIME으로 유저 기여도 분석
        
- 이후에 **딥러닝 방향으로 확장**하고 싶다면
    
    - MLP 기반 회귀 모델부터 시작
        
    - 유저 피처와 대학 평균 피처를 각각 입력받아 상호작용 학습
        
    - PyTorch or TensorFlow로 손쉽게 시도 가능
